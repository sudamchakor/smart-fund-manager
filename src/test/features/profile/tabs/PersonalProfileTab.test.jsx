...
jest.mock('../../../store/profileSlice', () => ({
  selectIncomes: jest.fn(() => [
    { id: 1, name: 'Salary', amount: 80000, frequency: 'monthly' },
  ]),
  selectProfileExpenses: jest.fn(() => [
    { id: 2, name: 'Groceries', amount: 20000, frequency: 'monthly' },
  ]),
...
describe('PersonalProfileTab Component', () => {
  ...
  it('renders incomes and expenses properly', () => {
    ...
    expect(screen.getByTestId('readonly-item-home-loan-emi')).toHaveTextContent(
      'Home Loan EMI',
    );
  });
  ...
  it('shows budget exceeded warning when surplus is negative', () => {
    ...
    expect(
      screen.getByText(
        /Your spending \(₹65,000\) exceeds income \(₹50,000\) by ₹10,000/i,
      ),
    ).toBeInTheDocument();
  });
});
