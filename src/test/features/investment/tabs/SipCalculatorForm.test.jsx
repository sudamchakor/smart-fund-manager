...
describe('SipCalculatorForm', () => {
    ...
    return render(
      <Provider store={store}>
        <ThemeProvider theme={theme}>
          <SipCalculatorForm sharedState={sharedState} onSharedStateChange={jest.fn()} />
        </ThemeProvider>
      </Provider>
    );
  };
  ...
  it('renders InputSliders with correct props and initial values', () => {
    renderComponent();
    // Monthly Investment
    expect(screen.getByTestId('mock-investment-slider-Monthly-Investment')).toBeInTheDocument();
    expect(screen.getByTestId('input-field-Monthly-Investment')).toHaveValue(10000);
    expect(screen.getByTestId('adornment-Monthly-Investment')).toHaveTextContent('₹');

    // Expected Return Rate
    expect(screen.getByTestId('mock-investment-slider-Expected-Return-Rate')).toBeInTheDocument();
    expect(screen.getByTestId('input-field-Expected-Return-Rate')).toHaveValue(12);
  ...
  it('calls onSharedStateChange when monthly investment changes', () => {
    ...
    expect(mockOnSharedStateChange).toHaveBeenCalledWith('monthlyInvestment', 15000);
  });
  ...
  it('calls onSharedStateChange when expected return rate changes', () => {
    ...
    expect(mockOnSharedStateChange).toHaveBeenCalledWith('expectedReturnRate', 15);
  });
